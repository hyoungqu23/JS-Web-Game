...
#### 4 X 4의 2차원 배열 생성하기
`start2048()`
가상의 태그인 fragment를 활용하는데, 이는 성능을 높이기 위해 사용된다. 즉, fragment가 없는 경우에는 한 번 반복문을 순회할 때마다 화면에 한 칸씩 표시하는데 비해, fragment는 화면이 아닌 메모리에만 저장되기 때문에 반복문 순회 중에 메모리에 표의 `<tr>`, `<td>`가 저장되고, 이를 화면에 나중에 한 번에 표시함으로써 더 나은 성능을 보장할 수 있다.

따라서 화면을 빈번하게 조작하는 경우 fragment를 활용하는 것이 더 좋다.


#### '2'를 임의의 위치에 삽입하는 함수 생성하기
`place2randomly()`

빈 칸에 임의로 2를 넣기 위해서는 빈 칸들 중 임의로 하나를 선택해야 한다. 빈 칸임을 확인하기 위해서는 cellData가 false 인 경우, 즉, 0인 경우도 포함하여 판별한다.

이때, 단순히 빈 칸을 선택하면, 2를 삽입하기 위해 해당 칸의 주소를 다시 찾아야 하기 때문에, 처음부터 빈 칸들을 판별할 때 그 칸의 주소를 배열에 저장한다.

따라서 forEach() method를 활용해 index 번호를 함께 저장하고, 이를 통해 쉽게 해당 칸에 2를 삽입할 수 있다.

#### 해당 칸의 데이터에 맞게 화면에 숫자 표시하기
`draw2048Table()`
2차원 배열을 순회하며, 해당 칸의 데이터가 0인 경우 칸을 비우고, 0이 아닌 2, 4, 8, ..., 1024, 2048인 경우에는 해당 칸에 데이터를 표시하고, 색상을 변경한다.
...
이때, dummy를 활용해 극단적인 예시 데이터를 삽입하여 확인하면서 코드를 작성하는 것이 좋다.
...
사람이 행동하는 것과 유사한 동작을 구현하면 된다. 즉, 2차원 배열인 경우 2회의 반복문을 통해 구현하고, 왼쪽 방향을 기준으로 할 때, 첫 줄에서 첫 칸이 빈 칸인 경우에 옆 칸의 데이터를 끌어오고, 해당 칸을 비우고, 다시 옆 칸을 확인하는 방식으로 코드를 구현하면 된다.

또한, 왼쪽 방향으로 정렬하는 경우, 칸에 데이터가 존재하고 옆 칸과 동일한 데이터를 가진다면, 이를 더하고, 해당 칸을 비우는 방식으로 코드를 구현하면 된다.


#### prevData 변수의 이해
forEach method에 의해 반복문을 순회하기 때문에 발생하는 것이다.
즉, dummy Data의 첫 줄인 `[0, 2, 4, 2]`의 경우를 확인해보면 newData가 다음과 같이 변경된다.

```js
newData = [];         // cellData = 0, !prevData
newData = [2];        // cellData = 2, prevData = 0
newData = [2, 4];     // cellData = 4, prevData = 2
newData = [2, 4, 2];  // cellData = 2, prevData = 4
```

이때 각 `cellData`와 `prevData`를 확인해보면, 이전 `newData` 배열의 마지막 값이 `prevData`가 되는 것을 알 수 있다. 따라서, 현재 `newData` 배열을 의미하는, `currentRow` 배열에서 마지막 요소가 이전 값, 즉 `prevData`가 된다.

#### `-2` 곱셈을 활용한 합치기에 대한 이해

같은 숫자 2개를 합칠 때, 기준 칸과 옆 칸의 숫자가 같으면 합치면 되는데, 이는 2, 2, 4, 8을 한 번에 16으로 합치는 오류*가 존재한다. 이를 또 dummy Data에 삽입하여 확인해볼 수 있다.

이러한 오류를 해결하는 방법으로 여러가지를 생각해볼 수 있는데, 그 중 하나가 -2를 곱하는 것을 통해 두 숫자를 합치는 것이다. 2와 2를 합치거나 곱하기 2를 하여 두 숫자를 합치는 경우, 4가 되어 다음 4와 일치하게 되기 때문에, -2를 곱하여 -4로 만들어 두 숫자를 다르게 만들어 줄 수 있다. 다만 이 경우 이후에 화면에 표시할 때는 Math.abs를 활용해 절대값으로 표시해야 한다.

#### case와 `{}`

case문에서 각 case를 작성할 때 중괄호(`{}`)를 반드시 작성해야 하는 것은 아니지만, 각 case 내부에서 변수를 선언하는 경우에는 변수가 중복되어 Error가 발생할 수 있다. 따라서, 중괄호(`{}`)를 활용해 블록을 설정해주어 이러한 문제를 해결할 수 있다.

 즉, case 'left' 말고도 'right', 'up', 'down'이 추가되고 그 안에도 `newData` 변수가 선언되기에 case에 중괄호(`{}`)가 없다면 같은 이름의 변수들이 같은 블록 내에 선언되는 셈이니 Error가 발생하게 된다.

### 005. 승리와 패배 구현하기

새롭게 극단적인 dummy Data를 만들어 왼쪽, 오른쪽 방향을 입력한 경우 승리를, 위, 아래 방향을 입력한 경우 패배를 하게끔 구현하고 코드를 작성하면 된다.
...
즉, 2048을 만드는 경우에는 승리, 빈 칸(즉, 0)이 없는 경우에는 패배, 둘 모두 아닌 경우에는 게임을 계속 진행해야 한다.
...
이때, 빈 칸이 없는 경우를 하나하나 검사해도 되지만, 더 효율적으로 확인하기 위해서 데이터를 확인하면 된다. 즉, 데이터와 화면을 일치시키기 위해 꾸준히 코드를 작성했기 때문에 화면에 빈 칸이 없는 것은 해당 2차원 배열의 데이터로 `0`이 없다는 것과 동일해지는 것이다.

### 006. 되돌리기 기능 추가하기
과거 데이터를 저장해두어야 되돌릴 수 있다.
...
2. 함수를 재사용하기 위해서는 외부 변수 참조를 최소화하는 것이 좋다.

3. 언어보다 설계가 중요하다. 즉, 설계하는 과정을 확실히 하면 언어로 코드를 작성하는 것은 추후에 결정할 일이다. 따라서, 코드를 보면서 어떻게 구현했는지를 설명하는 것이 아니라, 코드를 만들기 전에 어떻게 구현할 것인지를 설명할 줄 알아야 한다.
